...
class TaskPlannerNode(Node):
    def __init__(self):
        ...
        # NEW: position similarity tolerance (meters)
        ...
        self.latest_object = None  # now a Pose (from PoseArray.poses)
        self.latest_box = None     # now a Pose
        self.picked_xy = []        # NEW: list of (x,y) picked positions
    ...
    # NEW helper: coordinate similarity
    def _is_picked(self, x: float, y: float) -> bool:
        tol2 = self.pos_tol * self.pos_tol
        for px, py in self.picked_xy:
            dx = x - px
            dy = y - py
            if dx * dx + dy * dy <= tol2:
                return True
    ...
    # CHANGED: msg is PoseArray; choose first pose not already picked (by xy)
    def on_objects(self, msg: PoseArray):
        self.latest_object = None
        for p in msg.poses:
            x = float(p.position.x)
            y = float(p.position.y)
            if not self._is_picked(x, y):
                self.latest_object = p
                break

    # CHANGED: msg is PoseArray
    ...
    def step(self):
        ...
        if self.state == "NAV_TO_OBJECT":
            ...
            if self.pick_done:
                # CHANGED: remember picked by xy instead of id
                self.picked_xy.append((self.ox, self.oy))
