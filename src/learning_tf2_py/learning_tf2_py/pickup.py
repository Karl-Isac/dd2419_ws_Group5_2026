...
def is_red(h,s,v):          # Tuned for arm camera, not the same as the values used in detection
    return True if (h <= 20 or h >= 340) and s > 0.3 and v > 0.4 else False

def is_blue(h,s,v):
    return True if (h >= 180 and h <= 240) and s > 0.3 and v > 0.3 else False

def is_green(h,s,v):
    return True if 140 <= h <= 180 and s > 0.3 and v > 0.175 else False
